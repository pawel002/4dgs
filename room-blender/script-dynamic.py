...
def export_multi_camera_splatting():
    ...
    for cam_name in CAMERA_NAMES:
        ...
        for frame in range(scene.frame_start, scene.frame_end + 1):
            ...
            # ---> CRITICAL FIX: Extract Pose INSIDE the loop <---
            # Because the camera is moving, we must grab the new matrix on every frame
            matrix = cam.matrix_world.normalized()
            dynamic_transform_matrix = [list(row) for row in matrix]
            
            # Append to frames list with the updated matrix for this specific frame
            transforms_data["frames"].append({
                "file_path": f"images/{filename}",
                "transform_matrix": dynamic_transform_matrix
            })
