...
// each Gaussian.
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const vec3* means, vec3 campos, const float* shs, const bool* clamped, const vec3* dL_dcolor, vec3* dL_dmeans, vec3* dL_dshs)
{
	// Compute intermediate values, as it is done during forward
	vec3 pos = means[idx];
	vec3 dir_orig = pos - campos;
	vec3 dir = dir_orig / length(dir_orig);

	vec3* sh = ((vec3*)shs) + idx * max_coeffs;

	// Use PyTorch rule for clamping: if clamping was applied,
	// gradient becomes 0.
	vec3 dL_dRGB = dL_dcolor[idx];
	...
	vec3* dL_dsh = dL_dshs + idx * max_coeffs;
	...
	vec3 dL_ddir(dot(dRGBdx, dL_dRGB), dot(dRGBdy, dL_dRGB), dot(dRGBdz, dL_dRGB));
	...
	dL_dmeans[idx] += vec3(dL_dmean.x, dL_dmean.y, dL_dmean.z);
}
	...
	bool antialiasing)
{
	...
	mat3 J = mat3(h_x / t.z, 0.0f, -(h_x * t.x) / (t.z * t.z),
...
__device__ void computeCov3D(int idx, const vec3 scale, float mod, const vec4 rot, const float* dL_dcov3Ds, vec3* dL_dscales, vec4* dL_drots)
{
	// Recompute (intermediate) results for the 3D covariance computation.
	vec4 q = rot;// / length(rot);
	float r = q.x;
	float x = q.y;
	float y = q.z;
	float z = q.w;
	...
	vec3 dunc(dL_dcov3D[0], dL_dcov3D[3], dL_dcov3D[5]);
	vec3 ounc = 0.5f * vec3(dL_dcov3D[1], dL_dcov3D[2], dL_dcov3D[4]);
	...
	// dSigma_dM = 2 * M
	mat3 dL_dM = 2.0f * M * dL_dSigma;

	mat3 Rt = transpose(R);
	mat3 dL_dMt = transpose(dL_dM);

	// Gradients of loss w.r.t. scale
	vec3* dL_dscale = dL_dscales + idx;
	...
	float* dL_dopacity)
{
	...
	if (shs)
		computeColorFromSH(idx, D, M, (vec3*)means, *campos, shs, clamped, (vec3*)dL_dcolor, (vec3*)dL_dmeans, (vec3*)dL_dsh);
