...
def get_viewmat(optimized_camera_to_world):
    ...
    NOTE: torch.compile is intentionally disabled here. The viewer's render
    thread interrupts in-flight renders (IOChangeException) when the camera
    moves, which collides with dynamo/inductor compilation and crashes the
    render thread. This function is trivial, so eager execution is fine.
    """
...
class SplatfactoModelConfig(ModelConfig):
    ...
    "Size of the cube to initialize random gaussians within (fallback when no camera bbox)"
    random_init_from_camera_bbox: bool = True
    """When random-initializing, sample points inside the axis-aligned bounding box of all
    training-camera positions instead of a fixed-size cube. Falls back to `random_scale` if
    camera positions are unavailable or the bbox is degenerate (e.g. a single viewpoint)."""
    random_init_box_scale: float = 2.0
    """Factor by which the camera bounding box is expanded (per axis) when
    `random_init_from_camera_bbox` is enabled. 2.0 = a box twice the size of the camera bbox,
    centered on the cameras."""
...
class SplatfactoModel(Model):
    ...
    def _random_init_means(self) -> torch.Tensor:
        """Sample random initial gaussian positions.

        By default the points are drawn uniformly inside the axis-aligned bounding box of all
        training-camera positions, expanded by `random_init_box_scale` (so the cloud comfortably
        surrounds the cameras and the volume they observe). If camera positions are unavailable
        or the cameras are coincident (degenerate bbox, e.g. a single fixed viewpoint), we fall
        back to the original fixed-size `random_scale` cube centered at the origin.
        """
        num = self.config.num_random
        unit = torch.rand((num, 3)) - 0.5  # uniform in [-0.5, 0.5]^3

        cam_positions = None
        if self.config.random_init_from_camera_bbox:
            cam_positions = self.kwargs.get("metadata", {}).get("camera_positions", None)

        if cam_positions is not None and cam_positions.numel() > 0:
            cam_positions = cam_positions.reshape(-1, 3).to(dtype=unit.dtype, device=unit.device)
            bbox_min = cam_positions.min(dim=0).values
            bbox_max = cam_positions.max(dim=0).values
            size = bbox_max - bbox_min
            center = (bbox_max + bbox_min) / 2.0
            if torch.max(size) > 1e-6:
                box = size * self.config.random_init_box_scale
                CONSOLE.log(
                    f"Random init: sampling {num} gaussians in a box "
                    f"{self.config.random_init_box_scale}x the camera bbox "
                    f"(center={[round(c, 3) for c in center.tolist()]}, "
                    f"size={[round(s, 3) for s in box.tolist()]})."
                )
                return unit * box + center
            CONSOLE.log(
                "Random init: camera bounding box is degenerate (cameras coincident); "
                f"falling back to a {self.config.random_scale} cube."
            )

        return unit * self.config.random_scale

    def populate_modules(self):
        if self.seed_points is not None and not self.config.random_init:
            means = torch.nn.Parameter(self.seed_points[0])  # (Location, Color)
        else:
            means = torch.nn.Parameter(self._random_init_means())
