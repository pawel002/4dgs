...
from nerfstudio.data.datamanagers.temporal_datamanager import TemporalFullImageDatamanagerConfig
...
from nerfstudio.engine.temporal_trainer import TemporalTrainerConfig
...
from nerfstudio.models.temporal_splatfacto import TemporalSplatfactoModelConfig
...
descriptions = {
    ...
    "temporal-splatfacto": "Temporal (4D) Gaussian Splatting: one gaussian cloud per video frame from fixed cameras.",
}
...
method_configs["temporal-splatfacto"] = TemporalTrainerConfig(
    method_name="temporal-splatfacto",
    steps_per_eval_image=0,
    steps_per_eval_batch=0,
    steps_per_save=5000,
    steps_per_eval_all_images=1000000,
    # max_num_iterations is auto-derived from the temporal schedule by TemporalTrainer
    # (initial_iterations + (num_frames - 1) * tracking_iterations); this is a placeholder.
    max_num_iterations=100000,
    mixed_precision=False,
    pipeline=VanillaPipelineConfig(
        datamanager=TemporalFullImageDatamanagerConfig(
            dataparser=TemporalDataParserConfig(),
            cache_images="cpu",
            cache_images_type="uint8",
            initial_iterations=3000,
            tracking_iterations=300,
        ),
        model=TemporalSplatfactoModelConfig(
            initial_iterations=3000,
            tracking_iterations=300,
        ),
    ),
    optimizers={
        # The means learning rate is managed per-phase by the model (decays during
        # frame 0, constant during tracking), so no scheduler here.
        "means": {
            "optimizer": AdamOptimizerConfig(lr=1.6e-4, eps=1e-15),
            "scheduler": None,
        },
        ...
        "camera_opt": {
            "optimizer": AdamOptimizerConfig(lr=1e-4, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(lr_final=5e-7, max_steps=30000, warmup_steps=1000),
        },
        "bilateral_grid": {
            "optimizer": AdamOptimizerConfig(lr=2e-3, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-4, max_steps=30000, warmup_steps=1000),
        },
    },
    viewer=ViewerConfig(num_rays_per_chunk=1 << 15),
    vis="viewer",
