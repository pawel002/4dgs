...
class VanillaPipeline(Pipeline):
    ...
    def __init__(
        ...
        # Expose the training-camera positions to the model via metadata. splatfacto uses
        # these to size its random-init point cloud to the bounding box of all cameras.
        model_metadata = dict(self.datamanager.train_dataset.metadata)
        train_cameras = getattr(self.datamanager.train_dataset, "cameras", None)
        if train_cameras is not None and getattr(train_cameras, "camera_to_worlds", None) is not None:
            model_metadata.setdefault(
                "camera_positions", train_cameras.camera_to_worlds[..., :3, 3].detach().clone()
            )
