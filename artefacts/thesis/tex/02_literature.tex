...
3DGS represents a scene as a learnable point cloud of 3D Gaussians $\mathcal{G} = \{g_1, \dots, g_N\}$, where each primitive acts as a soft, semi-transparent ellipsoidal particle with learnable geometric and appearance parameters. The framework can be divided into several steps, presented in the following paragraphs. Because the remainder of this thesis --- and in particular the temporal method of Chapter~3 --- is notation-heavy, this section also fixes the parameterisation and symbols once, so that later chapters can build on them without re-deriving them.

\subsubsection{The Gaussian Primitive and its Parameterisation}

Each primitive is the tuple
\begin{equation}
...
\begin{itemize}
    ...
    $i$ at frame $t$. This is the parameter that motion acts on in Chapter~3.
    ...
    \begin{equation}
        ...
        = \mathbf{R}_i\, \mathbf{S}_i^2\, \mathbf{R}_i^{\!\top}.
        \label{eq:cov}
    \end{equation}
...
\begin{equation}
    \boldsymbol{\Sigma}^{\,2\mathrm{D}}_i = \mathbf{J}\,\mathbf{W}\, \boldsymbol{\Sigma}_i\, \mathbf{W}^{\!\top}\mathbf{J}^{\!\top},
\end{equation}
...
parameter $(\boldsymbol{\mu}_i, \mathbf{q}_i, \mathbf{s}_i, \tilde\sigma_i, \mathrm{SH}_i)$.

Finally, two indexing conventions are used throughout the thesis: $t \in \{0,\dots,F-1\}$
