...
The reconstruction of three-dimensional environments from two-dimensional observations has been studied since the early 1980s, and its evolution can be read as a sequence of paradigm shifts. First came the geometric era, in which reconstruction was a purely numerical exercise on matched pixel pairs. Then came the implicit-neural era, which replaced discrete geometry with continuous functions parametrised by deep networks. The most recent shift is the explicit radiance-field era, which reinstated explicit primitives while preserving differentiability. The method developed in this thesis builds on this explicit radiance-field paradigm and, additionally, engages with a fourth, currently active front, the extension of radiance-field methods to temporally varying scenes. This chapter reviews the four fronts in turn, giving extra weight to 3DGS (the immediate ancestor of the proposed method) and to the small but growing body of work on four-dimensional radiance fields (the direct comparison baseline), and closes with the formal mathematical background that Chapter~\ref{ch:research} will build upon.
...
\begin{figure}[!ht]
    ...
    \end{center}
    \caption{Figure adapted from Snavely et al.~\cite{10.1145/1179352.1141964}, showing how Bundler transforms a set of photos into a sparse 3D point cloud, which can be navigated and explored in 3D rendering software.}
    \label{fig:bundler}
\end{figure}

\subsection{COLMAP, the Current Geometric Gold Standard}
...
\begin{figure}[!ht]
    ...
    \end{center}
    \caption{Pipeline showing how 3D-R2N2 \cite{choy2016} transforms a set of images into a 3D voxel representation.}
    \label{fig:3d-r2n2}
\end{figure}
...
\noindent
Following the widespread adoption of 3DGS, four main temporal extensions have emerged.
...
\paragraph{Dynamic 3D Gaussians.} Luiten et al.~\cite{luiten2024dynamic} dispense with a global temporal model altogether and optimise the Gaussian set frame by frame, carrying the primitives forward in time so that each keeps its identity across the whole capture. Physical plausibility is maintained by explicit regularisers, a local-rigidity term that makes nearby Gaussians move like a rigid body, a rotation-similarity term, and a long-term isometry term, while colours and sizes are held fixed after the first frame. The persistent identity turns the reconstruction into a dense set of trackable trajectories. Of the four families this is the closest relative of the method proposed in this thesis, which shares the frame-by-frame structure and the fixed appearance but replaces the learned-free yet still elaborate regulariser stack with silhouette supervision and hull re-anchoring.

\subsection{Positioning of the Proposed Method}

The method developed in Chapter~\ref{ch:research} sits in a deliberately minimalistic corner of this design space. Unlike Deformable 3DGS and 4D-GS, it uses \emph{no} auxiliary neural network or temporal basis. Unlike SpaceTime Gaussians, it does \emph{not} store per-Gaussian polynomial coefficients. Like Dynamic 3D Gaussians, it preserves the bijection between frame-0 and frame-$t$ Gaussians, but it does so without any explicit rigidity or tracking loss, relying instead on a fixed topology, a first-order linear velocity prediction, and a hard freeze of the spherical-harmonic coefficients. The central claim of the thesis is that this ''less-is-more'' configuration is \emph{sufficient} for the characteristic regime of indoor multi-view video with static cameras, and empirically competitive with the more elaborate alternatives in that regime.

\section{Theoretical Background}

This section states the formal mathematical content of the two algorithms the thesis depends on.
\begin{itemize}
    \item COLMAP's incremental Structure-from-Motion, relevant when the method is applied to real-world captures.

    \item The differentiable rasterisation pipeline of 3D Gaussian Splatting.
\end{itemize}
...
3DGS represents a scene as a learnable point cloud of 3D Gaussians $\mathcal{G} = \{g_1, \dots, g_N\}$, where each primitive acts as a soft, semi-transparent ellipsoidal particle with learnable geometric and appearance parameters. The framework can be divided into several steps, presented in the following paragraphs. Because the remainder of this thesis, and in particular the temporal method of Chapter~\ref{ch:research}, is notation-heavy, this section also fixes the parameterisation and symbols once, so that later chapters can build on them without re-deriving them. Each primitive is the tuple
\begin{equation}
    g_i = \big(\boldsymbol{\mu}_i,\; \mathbf{s}_i,\; \mathbf{q}_i,\; \tilde\sigma_i,\; \mathrm{SH}_i\big),
\end{equation}
whose components carry the following meaning and storage convention (Figure~\ref{fig:gaussian}):

\begin{itemize}
    \item $\boldsymbol{\mu}_i \in \mathbb{R}^3$ is the \textbf{mean}, the centre of
    the primitive in world coordinates. When time matters it is written
    $\boldsymbol{\mu}_i(t)$ or $\boldsymbol{\mu}^{(i)}_t$, the position of primitive
    $i$ at frame $t$. This is the parameter that motion acts on in Chapter~\ref{ch:research}.
    ...
    logarithm guarantees a strictly positive scale for any real optimised value, so
    ...
    stored unnormalised and normalised at use, $\hat{\mathbf{q}}_i = \mathbf{q}_i /
    ...
    formula. Scale and rotation together define the $3\times3$ covariance, which is
    ...
    \begin{equation}
        ...
        \label{eq:cov}
    \end{equation}
    This factorisation is the reason orientation and scale are stored separately
    rather than as a raw covariance, it keeps $\boldsymbol{\Sigma}_i$ symmetric
    positive semi-definite by construction, which a freely optimised $3\times3$
    matrix would not be.

    \item $\tilde\sigma_i \in \mathbb{R}$ is the \textbf{pre-activation opacity}.
    The rendered opacity is the logistic sigmoid
    $\sigma_i = \mathrm{sig}(\tilde\sigma_i) = \big(1 + e^{-\tilde\sigma_i}\big)^{-1}
    \in (0,1)$, again so that any real optimised value yields a valid opacity.
    ...
    \mathbf{k}_{\ell,m}^{(i)} Y_{\ell}^{m}(\mathbf{d})$, with $Y_\ell^m$ the real SH
    basis. The zeroth band is the diffuse (Lambertian) term and higher bands capture
    ...
    \begin{equation}
        ...
        \label{eq:sh2rgb}
    \end{equation}
    where $\mathbf{1}=(1,1,1)^{\!\top}$ and $C_0$ is the normalisation of the
    constant SH basis function. Equation~\eqref{eq:sh2rgb} is used both to seed
...
The continuous density contributed by primitive $i$ at a world point $\mathbf{x}$ is
the unnormalised Gaussian
\begin{equation}
    G_i(\mathbf{x}) = \exp\!\Big(-\tfrac{1}{2}(\mathbf{x}-\boldsymbol{\mu}_i)^{\!\top}
    \boldsymbol{\Sigma}_i^{-1} (\mathbf{x}-\boldsymbol{\mu}_i)\Big).
\end{equation}
To form an image, each primitive is projected onto the image plane of a camera with
viewing transformation $\mathbf{W}$ and the Jacobian $\mathbf{J}$ of the local affine
approximation of the projective transformation, yielding a 2D Gaussian with covariance
\begin{equation}
    \boldsymbol{\Sigma}^{\,2\mathrm{D}}_i = \mathbf{J}\,\mathbf{W}\, \boldsymbol{\Sigma}_i\, \mathbf{W}^T\mathbf{J}^T,
\end{equation}
the elliptical-weighted-average (EWA) approximation. The screen is divided into pixel
tiles, primitives are culled against the view frustum and depth-sorted tile-by-tile
with a GPU radix sort. The colour of a pixel $\mathbf{p}$ is then the front-to-back
...
\begin{equation}
    ...
    \label{eq:alphablend}
\end{equation}
where $\mathcal{N}(\mathbf{p})$ is the depth-sorted set of primitives whose footprint
covers $\mathbf{p}$ and $G^{\,2\mathrm{D}}_i$ is the projected 2D Gaussian evaluated at
$\mathbf{p}$. This compositing rule is fully differentiable, gradients flow from the
pixel colour back through the depth-sort and the 2D covariance to every optimisable
parameter $(\boldsymbol{\mu}_i, \mathbf{q}_i, \mathbf{s}_i, \tilde\sigma_i, \mathrm{SH}_i)$. Finally, a few indexing conventions are used throughout the thesis. The symbol $t \in \{0,\dots,F-1\}$
indexes the $F$ temporal frames and $c \in \{1,\dots,C\}$ indexes the $C$ cameras of a
capture, $k$ indexes a global optimiser step, and a hat ($\hat I$) denotes a rendered
quantity as opposed to an observed one ($I$).

\subsubsection{Adaptive Density Control}

A critical feature that distinguishes 3DGS from methods operating on a fixed set of primitives is that the number of Gaussians is not predetermined. Instead, it evolves organically during training through a process of densification and pruning. This process is driven by the accumulated view-space positional gradient $\|\nabla_{\boldsymbol{\mu}} \mathcal{L}\|_2$, which serves as a proxy for reconstruction error. Regions of the scene that are poorly explained by the current set of Gaussians tend to accumulate large positional gradients as the optimizer repeatedly tries to shift primitives to reduce the loss. By monitoring this signal, the optimization can identify \emph{where} additional representational capacity is needed and allocate it adaptively, concentrating Gaussians in geometrically complex or high-frequency regions while leaving smooth, well-reconstructed areas sparse. The evolution strategy responds to two distinct and complementary failure modes, visualizations of which can also be seen in Figure \ref{fig:gaussian3}.
\begin{itemize}
    \item \textbf{Under-reconstruction} occurs in regions where Gaussians are too sparse to capture the underlying geometry, leaving gaps or missing detail. This case is diagnosed by a high positional gradient \emph{combined with} a small Gaussian scale, indicating that a small primitive is being pulled in multiple directions to cover more area than it can represent. It is resolved by \textbf{cloning}, that is, duplicating the chosen Gaussian and displacing the clone along the direction of the positional gradient, thereby increasing local density and total covered volume in the region that demands it.

    \item \textbf{Over-reconstruction} occurs in regions where a single large Gaussian attempts to cover complex geometry that varies at a finer scale than the primitive itself. This case is diagnosed by a high positional gradient \emph{combined with} a large Gaussian scale, and is resolved by \textbf{splitting}, that is, replacing the oversized Gaussian with two smaller children whose scale is reduced by a fixed factor and whose positions are sampled from the parent's own 3D distribution. In contrast to cloning, splitting preserves the covered volume while increasing spatial resolution, allowing finer detail to emerge.
\end{itemize}
In parallel with these growth operations, the representation is continuously kept compact. Gaussians whose opacity has decayed below a threshold are \textbf{pruned}, since near-transparent primitives contribute little to the rendered image while still incurring memory and compute cost. Additionally, every few thousand iterations all opacities are \textbf{reset} to a small ceiling value, which forces the optimization to re-justify the existence of each Gaussian and helps flush out spurious semi-transparent ''floaters'' that would otherwise persist in empty space. Together, densification, pruning, and opacity reset form a self-regulating loop that steers the model toward a small yet expressive set of primitives. This entire machinery is active during the static reconstruction phase of the pipeline described in Chapter~\ref{ch:research}. Crucially, the dynamic extension proposed by this thesis \emph{disables} densification, pruning, and opacity reset after the first frame, freezing the topology of the Gaussian set so that per-primitive temporal correspondence is preserved across the entire video sequence. Because each Gaussian retains a stable identity over time, its trajectory can be tracked and optimized frame-to-frame without the ambiguity that would arise if primitives were repeatedly created and destroyed. This design choice is motivated directly by the positioning discussed at the end of Section~\ref{sec:dynamic}.
\begin{figure}[!ht]
    \begin{center}
    \includegraphics[width=0.8\linewidth]{res/02_literature/gaussian-3.png}
    \end{center}
    \caption{Visualizations from the original paper \cite{kerbl2023} of how 3DGS solves the problem of under-reconstruction and over-reconstruction.}
    \label{fig:gaussian3}
\end{figure}
