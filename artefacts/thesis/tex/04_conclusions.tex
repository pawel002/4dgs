...
is reconstructed by a custom temporal method built inside the \emph{nerfstudio} framework.
...
The evaluation carries one built-in limit. All object-level
...
The main next step is the move from synthetic to real captures. The
...
That step is a data problem before it is an algorithmic one, and this is the
single sharpest limitation of the work. As
Section~\ref{sec:results-dataset-scope} sets out, the method consumes a specific
kind of recording, and the requirement that no existing capture happens to
satisfy is that every static camera must see the scene \emph{without} the main
subject at least once, so that a per-view empty-room plate exists. That plate is
what makes the pipeline model-free: it produces the silhouettes, which in turn
produce the visual-hull seed, the loss masks and the per-frame stray test, so
the whole chain is downstream of it. Published multi-view dynamic-scene datasets
begin recording with the performer already in the acting volume and therefore
provide no such view, which is why the benchmark of this thesis had to be
authored rather than borrowed, and why no measurement here is taken on real
footage. Acquiring a suitable real dataset is thus a piece of work in its own
right, and the protocol is straightforward to state, namely a rigidly mounted,
hardware-synchronized rig of a few cameras with locked exposure and white
balance, one calibration pass with a target, a short pass of the empty room
recorded from every camera before anyone enters, and only then the performance.
The alternative, consuming a conventional dynamic-capture dataset by replacing
the plate with a learned matting or segmentation network, is a legitimate and
probably necessary extension for reaching public benchmarks, but it should be
recognized for what it is, a different method whose accuracy is partly the
accuracy of a separately trained component, and it would need to be evaluated
against the plate-based version on a capture that supports both.
...
directly. This makes quantitative motion analysis possible on top of an ordinary
multi-camera recording. Gait analysis in rehabilitation, technique study in
sport, and ergonomic assessment on a factory floor all need this kind of dense,
markerless trajectory field, and the fixed calibrated rig the method
...
silhouette-supervised splatting pipeline. Holding topology and appearance fixed
turns a hard inverse problem into a tractable tracking problem and at the same
time makes the result compressible, which is a design choice available to dynamic
scene representations in general rather than a detail of this implementation.
